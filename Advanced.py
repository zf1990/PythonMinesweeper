...
class Advanced:
    ...
    def __init__(self, master):
        ...
        welcomeLabel = Label(frame, text = "Welcome to Advanced Minesweeper", fg = "blue")
        welcomeLabel.grid(row = 0, column = 0, columnspan = 30)
       ...
       #Placing mines.  The number x will serve as Tile ID.
        ...
        for x in range(0,380):
            if random.randrange(479) <= 98 and mines>0:
        ...
        for x in range(380,480): #Placing the rest of mines in the bottom tiles if there are still more mines to place.
            if mines > 0:
                mineStatus.append(True)
                mines=mines-1
            else:
        ...
        for TileID in range(0,480): # 30x16 tiles with buttons
            ...
            if (TileID-30) % 30 == 0: #To start a new row.  The entire size would be 9x9
                cordY = cordY + 1
                cordX = 0
        ...
        #Checking for mines
        for a in range(0,480): #Where a is equivalent to Button ID aka button widget.
    ...
    def getAdjacentCells(self, TileID):
        adjacentIDList = []
        if TileID == 0:
            adjacentIDList = [1, 30, 31]
            return adjacentIDList
        elif TileID == 29:
            adjacentIDList = [28, 59, 58]
        elif TileID == 450:
            adjacentIDList = [451, 420, 421]
        elif TileID == 479:
            adjacentIDList = [478, 449, 448]
        elif TileID > 0 and TileID < 29: #Top Row
            ...
            num5 = TileID + 31
            adjacentIDList = [num1,num2,num3,num4,num5]
        elif (TileID+1)%30 == 0: #Right Column
            ...
            num5 = TileID + 29
            adjacentIDList = [num1,num2,num3,num4,num5]
        elif TileID > 450 and TileID < 479: #Bottom Row
...
def main():
    ...
    root = Tk()
    # set program title
    root.title("Advanced Minesweeper")
    root.geometry("800x450")
    minesweeper = Advanced(root)
    # run event loop
    root.mainloop()
